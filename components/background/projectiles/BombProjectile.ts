...
export class BombProjectile extends Projectile {
  ...
  override update(sim: Simulation) {
      ...
      // Hit Black Hole
      if (distSq < visualRadiusSq && bh.state !== BHState.EXPLODING) {
          EffectSpawnService.createExplosion(this.x, this.y, 30, this.color, sim)
      ...
      if (sim.bigMeteor) {
          if (CollisionService.checkCircleCollision(this, sim.bigMeteor, 4, sim.bigMeteor.size)) {
              sim.bigMeteor.hp -= 1
              EffectSpawnService.createExplosion(this.x, this.y, 25, this.color, sim)
              this.markedForDeletion = true
              return
          }
      }

      // Hit Serpent (Instakill logic) - оптимизировано через SpatialGrid
      const nearbyEntities = sim.spatialGrid.query(this.x, this.y, 50)
      for (const entity of nearbyEntities) {
          if (entity instanceof VoidSerpent) {
              const s = entity as VoidSerpent
              if (CollisionService.checkSegmentCollision(this, s.segments, 30, 0)) {
                  DamageApplicationService.applyDamageToSerpent(s, 100, sim)
                  EffectSpawnService.createExplosion(this.x, this.y, 40, this.color, sim)
                  EffectSpawnService.spawnShockwave(this.x, this.y, sim)
                  this.markedForDeletion = true
                  return
              }
